...
この静的検索への切り替えには前提条件がありました。ゲーム検索に使っていた[IGDB](https://www.igdb.com/)のAPIには当然レート制限があり、データを丸ごとダンプして自前で配信するには通常のAPI利用規約の範囲では対応できません。そこで停止前日の2月25日、IGDB/Twitch側にデータパートナーシップを打診しました。結果としてパートナーシップを締結することができ、データダンプ機能が開放され、取得したデータのローカル保持も正式に許可されています。
